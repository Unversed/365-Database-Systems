...
SELECT termId, courseId, avg(grade) g
FROM Section 
JOIN Takes ON sectionId = id
group by termId, courseId
order by g desc;

SELECT t.id, startDate, endDate, max(gpa) FROM Term t 
      join (SELECT termId, courseId, avg(grade) gpa
      FROM Section 
      JOIN Takes ON sectionId = id
      group by termId, courseId) CrsGPA on termId = t.id
group by t.id;
...
(select firstName, lastName 
   ...
   where gpa > .9*(select max(gpa) from Student))
Union
(select firstName, lastName From Person p 
      join Teacher t on p.id = t.id
      join Teaches on teacherId = t.id 
      join Section s on sectionId = s.id 
      join Course c on courseId = c.id
      where c.avgGrade = (select min(avgGrade) from Course));
      
insert into Department(name) (select dept from Teacher) union (select dept from Section) union (select dept from Student);