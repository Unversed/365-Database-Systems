drop database if exists Courses2Agg;
create database Courses2Agg;
use Courses2Agg;
...
#SELECT firstName, lastName, unitsCompleted AS Claimed FROM Person join Student on Person.id = Student.id ORDER BY Student.id;

#SELECT studentId, sum(numUnits) FROM Takes join Section on sectionId = Section.id join Course on courseId = Course.id GROUP BY studentId ORDER BY studentId;


ALTER TABLE Takes 
ADD COLUMN grade INT(11);
UPDATE Takes 
SET grade = MOD((sectionId + studentId), 5);

ALTER TABLE Course 
ADD COLUMN avgGrade FLOAT(3, 2);

UPDATE Course 
SET avgGrade = (SELECT ROUND(avg(grade), 2) 
      FROM Takes join Section on sectionId = Section.id 
...
UPDATE Student
SET gpa = (SELECT ROUND((sum(numUnits*grade)/sum(numUnits)), 2) 
      FROM Takes join Section on sectionId = Section.id 
...
UPDATE Student
SET unitsCompleted = (SELECT sum(numUnits) 
      FROM Takes join Section on sectionId = Section.id 
      join Course on courseId = Course.id 
      WHERE grade > 0
