DROP DATABASE IF EXISTS Courses2;
CREATE DATABASE Courses2;
USE Courses2;
...
INSERT INTO `Term` VALUES 
   (1, 'W11', '2011-01-15', '2011-04-01', 38), 
   (2, 'S11', '2011-04-15', '2011-07-01', 36), 
   (3, 'F11', '2011-09-01', '2011-11-15', 31), 
   (4, 'W12', '2012-01-15', '2012-04-01', 32), 
   (5, 'S12', '2012-04-15', '2012-07-01', 36), 
   (6, 'F12', '2012-09-01', '2012-11-15', 30), 
   (7, 'W13', '2013-01-15', '2013-04-01', 24), 
   (8, 'S13', '2013-04-15', '2013-07-01', 36), 
   (9, 'F13', '2013-09-01', '2013-11-15', 30);
...
SELECT id, (SELECT count(*) FROM Course JOIN Section ON courseId = Course.id WHERE deptId = Department.id && numUnits = 2) AS '2 Units', 
      (SELECT count(*) FROM Course JOIN Section ON courseId = Course.id WHERE deptId = Department.id && numUnits = 3) '3 Units', 
      (SELECT count(*) FROM Course JOIN Section ON courseId = Course.id WHERE deptId = Department.id && numUnits = 4) '4 Units' 
FROM Department
ORDER BY id;

#course has one teacher in a given term
...
select distinct courseId from Section where
termId in (select id from Term where endDate > '2012-9-1');

SELECT  Distinct courseId FROM Section join term t on termId = t.id where endDate > '2012-9-1';