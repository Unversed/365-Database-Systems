DROP DATABASE IF EXISTS Bakery2M2Study;
CREATE DATABASE Bakery2M2Study;
USE Bakery2M2Study;
...
CREATE TABLE `Receipt` (
   `id` INT(11) PRIMARY KEY,  # NOT NULL is automatically implied
...
INSERT INTO `Discount` VALUES
   ...
   (4, '2007-10-20', '2007-10-28', 25),
   (5, '2007-10-26', '2007-10-26', 5),
   (6, '2007-10-29', '2007-10-30', 90);
...
/*List all ids of all products that appear in more than one receipt 
on the same day, in product id order.  */


SELECT DISTINCT L1.productId AS 'Product ID'
FROM Receipt R1, Receipt R2, LineItem L1, LineItem L2
WHERE R1.purchaseDate = R2.purchaseDate
&& R1.id <> R2.id
&& R1.id = L1.receiptId
&& R2.id = L2.receiptId
&& L1.productId = L2.productId
&& L1.receiptId <> L2.receiptId
ORDER BY L1.productId;

/*List all customers who have purchased a product 
that was subject to two discounts at the time of purchase. 
*/

SELECT DISTINCT lastName, firstName, Customer.id AS cid, Receipt.id AS rid
FROM Customer, Receipt, LineItem, DiscountXProduct DxP1, DiscountXProduct DxP2, Discount D1, Discount D2
WHERE customerId = Customer.id
&& receiptId = Receipt.id
&& LineItem.productId = DxP1.productId
&& DxP1.discountId = D1.id
&& DxP2.discountId = D2.id
&& DxP1.productId = DxP2.productId
&& DxP1.discountId <>DxP2.discountId
&& D1.id <> D2.id
&& D1.startDate <= purchaseDate && D1.endDate >= purchaseDate
&& D2.startDate <= purchaseDate && D2.endDate >= purchaseDate
ORDER BY Customer.id, Receipt.id;
...
INSERT INTO Kind(name) SELECT DISTINCT kind FROM Product ORDER BY kind;
INSERT INTO Flavor(name) SELECT DISTINCT flavor FROM Product ORDER BY flavor;

ALTER TABLE Product
ADD kindId INT NOT NULL AFTER id,
ADD flavorId INT NOT NULL AFTER id;

UPDATE Product 
SET kindId = (SELECT id FROM Kind WHERE kind = Kind.name);
UPDATE Product 
SET flavorId = (SELECT id FROM Flavor WHERE flavor = Flavor.name);
...
Alter TABLE Product 
ADD CONSTRAINT FKProduct_flavorId FOREIGN KEY(flavorId)
REFERENCES Flavor(id),
ADD CONSTRAINT FKProduct_kindId FOREIGN KEY(kindId)
REFERENCES Kind(id),
ADD UNIQUE KEY UKkindId_flavorId (`kindId`, `flavorId`);



CREATE TABLE `CustomerXKind` (
   `customerId` INT(11) NOT NULL,
   `kindId` INT NOT NULL,
   PRIMARY KEY (`customerId`, `kindId`),
   CONSTRAINT `FKCustomerXKind_customertId` FOREIGN KEY (`customerId`) 
      REFERENCES `Customer` (`id`),
   CONSTRAINT `FKCustomerXKind_kindId` FOREIGN KEY (`kindId`) 
      REFERENCES `Kind` (`id`)
);

CREATE TABLE `CustomerXFlavor` (
   `customerId` INT(11) NOT NULL,
   `flavorId` INT NOT NULL,
   PRIMARY KEY (`customerId`, `flavorId`),
   CONSTRAINT `FKCustomerXFlavor_customerId` FOREIGN KEY (`customerId`) 
      REFERENCES `Customer`(`id`),
   CONSTRAINT `FKCustomerXFlavor_flavorId` FOREIGN KEY (`flavorId`) 
      REFERENCES `Flavor`(`id`)
);

INSERT INTO CustomerXKind(customerId, kindId)
SELECT Customer.id, Kind.id 
FROM Customer, Kind
WHERE LOCATE(Kind.name, favorites collate utf8_bin) > 0;

INSERT INTO CustomerXFlavor(customerId, flavorId)
SELECT Customer.id, Flavor.id
FROM Customer, Flavor
WHERE LOCATE(Flavor.name, favorites collate utf8_bin) > 0;
