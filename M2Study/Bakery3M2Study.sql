DROP DATABASE IF EXISTS Bakery3M2Study;
CREATE DATABASE Bakery3M2Study;
USE Bakery3M2Study;
...
select customerId, productId, count(*) from Rating group by customerId, productId;
select customerId, productId, count(*) from Rating group by productId;
...
GROUP BY Customer.id ORDER BY sum(price) DESC; 



SELECT kind, GROUP_CONCAT(flavor ORDER BY flavor SEPARATOR ',') 
FROM Product
GROUP BY kind;

#Midterm 1

SELECT id AS 'Id', firstName AS 'First', lastName AS 'Last' 
FROM Customer
WHERE (LOCATE('Strawberry', favorites) > 0
&& !(LOCATE(' Berry', favorites) > 0))
&& !((state = 'CA' || state = 'OR'|| state = 'WA') && (age < 18 || age > 65))
ORDER BY Last, First, Id;

SELECT firstName, lastName FROM Customer, Rating, Product 
WHERE customerId=Customer.id 
&& productId = Product.id 
&& price > 10.00
&& LOCATE('cake', comment) > 0
ORDER By lastName, firstName;

SELECT kind, flavor 
FROM Product, Rating r1, Rating r2
WHERE r1.productId=id 
&& r2.productId = id
&& r2.score > 6 
&& r1.score < 4
&& r2.productId = r1.productId
ORDER BY kind, flavor;


SELECT DISTINCT p1.id, d1.id, p2.id, d2.id 
FROM Product p1
join DiscountXProduct dp1 on p1.id = dp1.productId 
join Discount d1 on dp1.discountId = d1.id
join Product p2 
join DiscountXProduct dp2 on p2.id = dp2.productId 
join Discount d2 on dp2.discountId = d2.id
WHERE p1.id <> p2.id && d1.id = d2.id
