DROP DATABASE IF EXISTS Bakery1M2Study;
CREATE DATABASE Bakery1M2Study;
USE Bakery1M2Study;
...
FROM Customer
WHERE favorites LIKE '%Cake%'
|| favorites LIKE '%Chocolate%';

SELECT firstName, lastName
FROM Customer
WHERE age < 18
&& gender = 'F'
&& favorites NOT LIKE '%chocolate%'
&& favorites NOT LIKE '%strawberry%'
ORDER BY id;

SELECT firstName, lastName
FROM Customer
WHERE state IN ('AK', 'AL', 'CA', 'CT', 'DE', 'FL', 'GA', 'HI', 'LA', 'MA',
 'MD', 'ME', 'MS', 'NC', 'NH', 'NJ', 'NY', 'OR', 'RI', 'SC', 'TX', 'VA', 'WA');
 
SELECT firstName AS 'First Name', lastName AS 'Last Name', age AS 'Age'
FROM Customer
WHERE (age >= 65 
|| age < 18)
&& ((YEAR(lastVisit) = '2012' 
...
SELECT firstName, lastName
FROM Customer
WHERE favorites LIKE '% berry,%' 
|| favorites LIKE 'berry,%'
ORDER BY id;
