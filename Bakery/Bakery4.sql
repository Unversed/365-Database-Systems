...
SELECT firstName, lastName FROM Customer WHERE id = (SELECT max(id)	 FROM Customer);

select firstName, lastName, (select (max(price)-min(price)) 
      from Receipt join LineItem on receiptId = id 
      join Product on Product.id = productId 
      where customerId = Customer.id) AS Spread
From Customer
WHERE (select (max(price)-min(price)) 
      from Receipt join LineItem on receiptId = id 
      join Product on Product.id = productId 
      where customerId = Customer.id) IS NOT NULL
ORDER BY (select max(price) 
      from Receipt join LineItem on receiptId = id 
      join Product on Product.id = productId 
      where customerId = Customer.id) Desc;
...
SELECT avg(score) FROM Rating Group By productId order by productId;

SELECT id, avg(score) AS avg 
FROM Rating a JOIN Product ON a.productId = Product.id 
...
SELECT distinct id FROM Customer join Rating on customerId = Customer.id Group By customer.id having max(score) IS NULL;

SELECT max(score) FROM Rating join customer on customerId = customer.id group by customer.id;


SELECT DISTINCT id FROM Customer c1 JOIN Rating r1 ON customerId = c1.id 
Where (SELECT max(score) 
      FROM Rating r2 
      WHERE r1.customerId = r2.customerId 
      GROUP BY r1.customerId 
      ORDER BY r1.customerId) < 6
            || (select (select max(score) 
            From Rating r2 
            where r1.customerId = c1.id) 
            FROM Customer c2 WHERE c1.id = c2.id) IS NULL
ORDER BY id;

SELECT id 
FROM Customer c1 
WHERE (SELECT (SELECT MAX(score) 
            FROM Rating 
            WHERE customerId = c1.id) 
      FROM Customer c2 
      WHERE c1.id=c2.id) IS NULL
|| (SELECT MAX(score) 
      FROM Rating r1
      WHERE customerId = c1.id) < 6
ORDER BY id;
...
(select sum(price) from Receipt join LineItem on
receiptId = id join Product on Product.id =
productId where customerId = Customer.id) > 100;

SELECT firstName, lastName FROM Customer c
JOIN Receipt r ON customerId = c.id
JOIN LineItem ON receiptId = r.id
...
SELECT * 
FROM Product p
WHERE price <= any (SELECT price FROM Receipt r
JOIN LineItem ON receiptId = r.id
WHERE productId = p.id && customerId = 2);


select * from Product p where exists
(select * from Product where flavor = 'Chocolate' and price > p.price);

select * from Product p where (select count(*) from Product where flavor = 'Chocolate' and price > p.price);

SELECT DISTINCT flavor, kind FROM Product p
JOIN DiscountXProduct on productId = p.id
JOIN Discount d ON discountId = d.id
WHERE startDate <= '2007-10-26'
&& endDate >= '2007-10-26';

SELECT flavor, kind 
FROM Product p
WHERE p.id NOT IN (SELECT p2.id FROM Product p2
      JOIN DiscountXProduct on productId = p2.id
      JOIN Discount d ON discountId = d.id
      WHERE startDate <= '2007-10-26'
      && endDate >= '2007-10-26');
...
SELECT flavor, kind 
FROM Product p
WHERE p.id != ALL (SELECT p2.id FROM Product p2
      JOIN DiscountXProduct on productId = p2.id
      JOIN Discount d ON discountId = d.id
      WHERE startDate <= '2007-10-26'
      && endDate >= '2007-10-26');
...
select kind, flavor, price from Product where
price > any (select price from Product where flavor = 'Chocolate');

select min(price) from Product;

select p1.kind, p1.flavor, p1.price from Product p1 join Product p2  where p1.price > p2.price && p2.flavor = 'Chocolate' group by p1.id;


select kind, flavor, price from Product
where price > any (select price from Product where flavor = 'Chocolate');

select kind, flavor, price from Product
where not price <= all (select price from Product where flavor = 'Chocolate');

#SUBQUERIES C	

select state, max(rAvg) from customer 
join (select customerId, avg(total) rAvg from Receipt group by customerId) ctot on id = customerId 
group by state;

#This treats the inline view as a separate table, 
#joining it with Receipt and Customer to link the max prices with Customer data
#then grouping by customer ID.

select firstName, lastName, avg(hiPrice) from customer 
join Receipt r on customerId = Customer.id
join (select receiptId rId, max(price) hiPrice 
      from LineItem 
      join Product p on productId = p.id 
      group by receiptId 
      having count(*) > 1) rHi on r.id = rId
group by customerId;

#SUBQUERIE D

(select id, firstName, lastName from Customer
where age > 18 and state in ('CA', 'OR', 'WA'))
UNION
(select id, firstName, lastName from Customer where age < 50);

select id, firstName, lastName from Customer
where (age < 50 || age > 18 and state in ('CA', 'OR', 'WA'));