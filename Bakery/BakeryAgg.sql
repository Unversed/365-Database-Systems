DROP DATABASE IF EXISTS BakeryAgg;
CREATE DATABASE BakeryAgg;
Use BakeryAgg;
...
CREATE TABLE `LineItemNew` (
   `receiptId` INT(11) DEFAULT NULL,
   `lineNum` INT(11) DEFAULT NULL,
   `productId` VARCHAR(20) DEFAULT NULL,
   `qty` INT(1) DEFAULT NULL,
   `extPrice` DECIMAL(6,2) DEFAULT NULL
);

INSERT INTO LineItemNew (receiptId, lineNum, productId, qty) 
SELECT receiptId, lineNum, productId, COUNT(productId) 
FROM LineItem 
GROUP BY receiptId, productId 
ORDER BY receiptId, LineNum;

UPDATE LineItemNew SET extPrice = (SELECT qty * price FROM Product WHERE productId = Product.id);

DROP TABLE LineItem;

RENAME TABLE LineItemNew to LineItem;

ALTER TABLE LineItem
ADD CONSTRAINT `FKLineItem_receiptId` FOREIGN KEY (`receiptId`)
      REFERENCES `Receipt` (`id`),
ADD CONSTRAINT `FKLineItem_productId` FOREIGN KEY (`productId`) 
      REFERENCES `Product` (`id`);
...
UPDATE Receipt 
SET total = (SELECT sum(extPrice) 
      FROM LineItem 
      WHERE receiptId = Receipt.id 
      GROUP BY receiptId);
SELECT * FROM Receipt;

SELECT flavor, kind, sum(qty) AS 'Orders' 
FROM LineItem join Product on productId = Product.id 
GROUP BY productId 
ORDER BY productId; 

SELECT flavor, avg(age) AS 'Age', Count(DISTINCT Customer.id) AS 'Popularity' 
FROM Customer join Receipt on customerId = Customer.id
join LineItem on receiptId = Receipt.id
join Product on productId = Product.id 
GROUP BY flavor
HAVING Count(DISTINCT Customer.id) > 10
ORDER BY avg(age);

SELECT flavor, kind, GROUP_CONCAT(DISTINCT state ORDER BY state SEPARATOR ' ') AS 'States'
FROM Customer join Receipt on customerId = Customer.id
join LineItem on receiptId = Receipt.id
join Product on productId = Product.id 
GROUP BY Product.id
...
SELECT EXTRACT(MONTH FROM purchaseDate) AS 'Month', EXTRACT(DAY FROM purchaseDate) AS 'Day', kind, sum(qty) AS 'Count'
FROM Receipt join LineItem on receiptId = Receipt.id
join Product on productId = Product.id
GROUP BY kind, EXTRACT(MONTH FROM purchaseDate), EXTRACT(DAY FROM purchaseDate)
HAVING sum(qty) > 1
ORDER BY Month, Day, kind;
