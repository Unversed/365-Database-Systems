DROP DATABASE IF EXISTS Bakery4OJ;
CREATE DATABASE Bakery4OJ;
USE Bakery4OJ;
...
(select * from LineItem where id = productId);

select id from Product left join LineItem on id = productId where productId is null;


select id from Customer c where not exists
(select * from Receipt where c.id = customerId);

select Customer.id from Customer left join Receipt on Customer.id = customerId where customerId is null;



select * from DiscountXProduct join Product
on id = productId where flavor = 'Chocolate';

select id 
from Discount d 
where not exists (select discountId 
      from DiscountXProduct
      join Product p on p.id = productId 
      where discountId = d.id 
      &&  flavor = 'Chocolate');

select d.id from Product p join DiscountXProduct
on id = productId and flavor = 'Chocolate'
right join Discount d on d.id = DiscountId
where p.id is null;
...
left join (select * 
      from Product 
      join DiscountXProduct on id = productId 
      and flavor = 'Chocolate') cp on d.id = discountId
where cp.id is null;

select distinct flavor, kind from Product p join LineItem where productId = p.id && qty = 1;





#customers with rating scores > 5
...
SELECT id
FROM Customer c
LEFT JOIN Rating r ON c.id = customerId && score > 5
...
SELECT c.id, l.productId, qty
FROM Customer c
JOIN Receipt r ON r.customerId=c.id
JOIN LineItem l ON receiptId = r.id
ORDER BY c.id, l.productId;
...
#all products purchased without rating
SELECT Distinct c.id 'Customer', l.productId 'Product'
FROM Customer c
JOIN Receipt r ON r.customerId=c.id
JOIN LineItem l ON receiptId = r.id
LEFT JOIN Rating on Rating.customerId = c.id && Rating.productId = l.productId
WHERE Rating.productId IS NULL
ORDER BY c.id, l.productId;
...
SELECT r.id, l.productId
FROM Receipt r
JOIN LineItem l on receiptId = r.id
Order BY l.productId, r.id;


#all products purchased by a customer
SELECT customerId, r.id, l.productId
FROM Receipt r
JOIN LineItem l on receiptId = r.id
Order BY customerId, l.productId, r.id;



SELECT c.id 'Customer', l.productId 'Product'
FROM Customer c
JOIN Receipt r on customerId = c.id
JOIN LineItem l on receiptId = r.id
LEFT JOIN (SELECT customerId, id, productId
      FROM Receipt r
      JOIN LineItem ON receiptId = r.id) Rlist ON r.id <> Rlist.id 
&& Rlist.customerId = c.id 
&& l.productId = Rlist.productId
WHERE Rlist.id IS NULL && l.qty = 1
Order BY c.id, l.productId;
...
#all products receipts which share a product
SELECT DISTINCT c1.id, r1.id, r2.id, l1.productId
FROM Customer c1
JOIN Receipt r1 ON r1.customerId = c1.id
JOIN LineItem l1 ON l1.receiptId = r1.id
JOIN Customer c2 ON c1.id = c2.id
JOIN Receipt r2 ON r2.customerId = c2.id && r1.id <> r2.id
JOIN LineItem l2 ON l2.receiptId = r2.id
ORDER BY c1.id, r1.id, l1.productId;
