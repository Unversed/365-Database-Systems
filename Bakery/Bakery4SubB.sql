DROP DATABASE IF EXISTS Bakery4SubB;
CREATE DATABASE Bakery4SubB;
USE Bakery4SubB;
...
#items purchased 6 or less times
select p.id, (select sum(qty) 
      from Lineitem 
      where productId = p.id 
      group by productId) numBought 
from Product p 
having numBought <= 6 
|| numBought is null;

#customers who have not given a rating
SELECT id FROM Customer c where not exists (SELECT customerId FROM Rating where c.id = customerId);

#number of times an item has been purchased
select p.id, (select sum(qty) from lineitem where productId = p.id group by productId) numBought from Product p; 


#number of times a product has been rated
select p.id, (select count(*) from rating where productId = p.id group by productId) numRate from Product p; 



#(number of times a product has been bought - number of times it has been rated) < 6
...
FROM (SELECT p.id, 
            ...
            WHERE productId = p.id 
            GROUP BY productId) numBought 
      FROM Product p) Bought
JOIN (SELECT p.id, (SELECT count(*) 
            FROM Rating 
            WHERE productId = p.id 
            GROUP BY productId) numRate 
      FROM Product p) Rates ON Bought.id = Rates.id
WHERE IFNULL(numBought,0) - IFNULL(numRate,0) <= 6
ORDER BY Bought.id;
...
SELECT p.id, (SELECT count(*)
      FROM Receipt r
      JOIN LineItem l ON receiptId = r.id
      WHERE productId = p.id 
      && NOT EXISTS (SELECT customerId FROM Rating where r.customerId = Rating.customerId)
      GROUP BY productId) CCounts
FROM Product p
ORDER BY p.id;
...
SELECT count(*)
      FROM Receipt r
      JOIN LineItem l ON receiptId = r.id
#      WHERE NOT EXISTS (SELECT customerId FROM Rating where r.customerId = Rating.customerId)
      GROUP BY productId;


SELECT c.id, productId, count(Distinct c.id)
      FROM Customer c
...
SELECT id
FROM (SELECT p.id, (SELECT count(Distinct c.id) 'count'
            FROM LineItem
            JOIN Receipt r ON receiptId = r.id
            ...
            WHERE NOT EXISTS (SELECT customerId 
                  ...
                  && productId = p.id)
            && productId = p.id
            GROUP BY productId) 'countz'
      FROM Product p) Customers
WHERE countz <= 6
|| countz IS NULL
ORDER BY id;
...
#customers that have done atleast 2 ratings
...
#products that have recieved atleast 2 ratings
SELECT productId
FROM Rating
GROUP BY productId
HAVING count(*) >= 2;

SELECT customerId 'Customer', productId 'Product', score
FROM Rating
...
GROUP BY productId
HAVING count(*) >= 2)
ORDER BY Customer, Product, score;